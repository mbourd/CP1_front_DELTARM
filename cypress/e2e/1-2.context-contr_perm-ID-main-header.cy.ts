...
describe(
  'Assert interface #main-header context contr_perm',
  { testIsolation: false },
  () => {
    beforeEach(() => {
      cy.intercept({
    ...
    it('Assert attr title if icon visible', () => {
      const trans_EN1 = _translate('en', 'MainHeader', 'reports') || 'reports';
      const trans_FR1 = _translate('fr', 'MainHeader', 'reports') || 'reports';
      const trans_DE1 = _translate('de', 'MainHeader', 'reports') || 'reports';
      const translation1 = [trans_EN1, trans_FR1, trans_DE1];
      const trans_EN2 =
        _translate('en', 'MainHeader', 'F.A.Q') || _escapeForRegExp('F.A.Q');
      const trans_FR2 =
        _translate('fr', 'MainHeader', 'F.A.Q') || _escapeForRegExp('F.A.Q');
      const trans_DE2 =
        _translate('de', 'MainHeader', 'F.A.Q') || _escapeForRegExp('F.A.Q');
