...
describe('ApiRouter', () => {
  ...
  it('should register route', () => {
    expect(apiRouter.getRoutes()).toStrictEqual({
      [userRouteDef.name]: userRouteDef,
    });
  });

  it('should generate path without queries', () => {
    expect(apiRouter.generatePath('GetUser', { id: 2 })).toStrictEqual(
      '/user/2',
    );
  });
  ...
  it('should generate path with  queries', () => {
    ...
    ).toStrictEqual('/user/2?name=john&count=42');
  });
