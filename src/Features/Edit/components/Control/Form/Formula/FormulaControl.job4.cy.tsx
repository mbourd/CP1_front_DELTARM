...
// NOTE: Run CLI:
// yarn cypress:run:component --browser chrome --config video=false --spec "src/Features/Edit/components/Control/Form/Formula/FormulaControl.job4.cy.tsx"
...
describe('<FormulaControl />', () => {
    ...
    cy.mount(
      ...
      .then(() => {
        cy.window()
          .then((window) => {
            window[
              'Features/Edit/Control/Form/Formula/FormulaControl'
  ...
  it('should render <ControlLabel /> and <ControlFooter/>', () => {
    ...
    cy.waitReactApp();
    cy.react('FormulaControl').react('ControlLabel');
    cy.react('FormulaControl').react('ControlFooter');
  });
  ...
  it('should be disabled', () => {
    ...
    cy.waitReactApp();
    cy.react('FormulaControl').find('input[type="text"]').should('be.disabled');
  });
  it('should be disabled', () => {
    const _control: IApiControl = {
      ...structuredClone(control),
  ...
  it('should render <RejectControl /> if useRejection & control_rejectable', () => {
    ...
    cy.waitReactApp();
    cy.react('FormulaControl').react('RejectControl');
  });
});
