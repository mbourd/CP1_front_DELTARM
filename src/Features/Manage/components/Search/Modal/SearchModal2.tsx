...
export const SearchModal: React.FC<IProps> = ({ onClose, open }): React.ReactElement | null => {
  ...
  if (data && data.error && !data.fileId && route?.type === 'DRM_CREATE') {
    return (
      <Modal open={open} onClose={onClose}>
  ...
  if (data && data.error && !data.fileId && route?.type === 'DRM') {
    const footer = (
  ...
  if (data && data.error && !data.file && route?.type === 'KSIOP') {
    return (
      <Modal open={open} onClose={onClose}>
  ...
  if (data && !data.error && data.file && data.productList && route?.type === 'KSIOP') {
