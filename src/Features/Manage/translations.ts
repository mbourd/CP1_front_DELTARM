import { translation, useTrans } from 'Services';
import { useCallback } from 'react';

const namespace = 'Manage';
const transManage = {
  pageTitle: {
    en: 'Manage',
    fr: 'Gestion',
    de: 'Verwalten',
  },
  searchPlaceholder: {
    en: 'Find a file',
    fr: 'Rechercher un dossier (N°Dossier / N°Avenant)',
    de: 'Suchen Sie eine Datei',
  },
  filter: {
    en: 'Filter',
    fr: 'Filtrer',
    de: 'Filter',
  },
  sort: {
    en: 'Sort',
    fr: 'Trier',
    de: 'Sortieren',
  },
  edit: {
    en: 'Edit',
    fr: 'Editer',
    de: 'Bearbeiten',
  },
  readComments: {
    en: 'Read reviews',
    fr: 'Lire les commentaires',
    de: 'Bewertungen lesen',
  },
  classify: {
    en: 'Classify without continuation',
    fr: 'Classer sans suite',
    de: 'Klassifizieren ohne Fortsetzung',
  },
  searchButtonLabel: {
    en: 'Find file',
    fr: 'Lancer la recherche',
    de: 'Datei finden',
  },
  applyFilter: {
    en: 'Apply filters',
    fr: 'Appliquer les filtres',
    de: 'Filter anwenden',
  },
  numberComments: {
    en: 'Number of comments',
    fr: 'Nombre de commentaires',
    de: 'Anzahl der Kommentare',
  },
  resetFilterButtonLabel: {
    en: 'Reset filters',
    fr: 'Réinitialiser les filtres',
    de: 'Filter zurücksetzen',
  },
  empty: {
    en: 'No files available',
    fr: 'Pas de dossiers disponible',
    de: 'Keine Dateien verfügbar',
  },
  searchError: {
    en: 'No files available',
    fr: "Le format de la recherche n'est pas valide.",
    de: 'Keine Dateien verfügbar',
  },
  searchByCounterPartyOrUser: {
    en: 'Search by counterparty or user',
    fr: 'Rechercher par contrepartie ou utilisateur',
    de: 'Suche nach Kontrahenten oder Benutzern',
  },
  searchByNumber: {
    en: 'Search by number',
    fr: 'Rechercher par numéro',
    de: 'Suche nach Nummer',
  },
  counterpartyBorrowerOrSurname: {
    en: 'Counterparty borrower or surname',
    fr: 'Contrepartie emprunteuse ou nom de famille',
    de: 'Gegenpartei Kreditnehmer oder Nachname',
  },
  filterFolders: {
    en: 'Filter folders',
    fr: 'Filtrer les dossiers',
    de: 'Ordner filtern',
  },
  mandatoryFields: {
    en: 'Mandatory fields missing',
    fr: 'Champs obligatoire manquant',
    de: 'Pflichtfelder fehlen',
  },
  cancelCreation: {
    en: 'Cancel creation',
    fr: 'Annuler la création',
    de: 'Erstellung abbrechen',
  },
  confirmCreation: {
    en: 'Confirm creation',
    fr: 'Confirmer la création',
    de: 'Erstellung bestätigen',
  },
  selectProductFamily: {
    en: 'Select a product family',
    fr: 'Sélectionner une famille de produit',
    de: 'Produktfamilie aus',
  },
  serverNotResponding: {
    en: 'The server is not responding',
    fr: 'Le serveur ne répond pas',
    de: 'Der Server antwortet nicht',
  },
  responseFromKSIOP: {
    en: 'Response from KSIOP',
    fr: 'Réponse de KSIOP',
    de: 'Antwort von KSIOP',
  },
  fileNotFound: {
    en: 'File not found!',
    fr: 'Dossier introuvable !',
    de: 'Datei nicht gefunden!',
  },
  Sélectionner: {
    en: 'Select',
    fr: 'Sélectionner',
    de: 'Auswählen',
  },
  noResult: {
    en: 'No result',
    fr: 'Aucun résultat',
    de: 'Kein Ergebnis',
  },
  cancelSearch: {
    en: 'Cancel search',
    fr: 'Annuler la recherche',
    de: 'Suche abbrechen',
  },
  resultOfTheSearch: {
    en: 'Result of the research',
    fr: 'Résultat de la recherche',
    de: 'Ergebnis der Recherche',
  },
  cancel: {
    en: 'cancel',
    fr: 'Annuler',
    de: 'abbrechen',
  },
  goToVerification: {
    en: 'Go to verification',
    fr: 'Accéder à la validation',
    de: 'Gehen Sie zur Überprüfung',
  },
  editFolder: {
    en: 'Edit Folder',
    fr: 'Editer le dossier',
    de: 'Ordner bearbeiten',
  },
};
translation.addBatchResource({
  Manage: transManage,
});
type UseTransManageReturnType = {
  trans: (key: keyof typeof transManage, options?: Record<any, any>) => string;
  transManage: typeof transManage;
  currentLang: string | undefined;
  changeLang: (lang: string) => void;
};
export const useTransManage = (): UseTransManageReturnType => {
  const [trans, changeLang, currentLang] = useTrans(namespace);
  const translate = useCallback(
    (key: keyof typeof transManage, options?: any): string =>
      trans(key, options),
    [trans],
  );

  return { trans: translate, transManage, currentLang, changeLang };
};
